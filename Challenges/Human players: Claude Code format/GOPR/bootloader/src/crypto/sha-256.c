...
void sha_256_write(struct Sha_256 *sha_256, const void *data, uint32_t len)
{
	...
	while (len > 0) {
		...
		const uint32_t consumed_len = len < sha_256->space_left ? len : sha_256->space_left;
...
uint8_t *sha_256_close(struct Sha_256 *sha_256) {
	...
	uint32_t space_left = sha_256->space_left;
	...
	*pos++ = 0x80;
	...
	const uint32_t left = space_left - TOTAL_LEN_LEN;
...
void calc_sha_256(uint8_t hash[SIZE_OF_SHA_256_HASH], const void *input, uint32_t len) {
